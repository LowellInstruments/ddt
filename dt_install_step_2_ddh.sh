...
GH_REPO_DDH=https://github.com/lowellinstruments/ddh5.git
F_CLONE_MAT=/tmp/mat
F_CLONE_DDH=/tmp/ddh
...
#    cp $F_CLONE_MAT/tools/_setup_wo_reqs.py $F_CLONE_MAT/setup.py
    "$VPIP" install --no-dependencies $F_CLONE_MAT
    ...
    _pb "[ 50% ] DDH requirements"
    # useful for debug
    if [ -d "$FOL_LI"/ddh ]; then
        _pb "already detected DDH folder"
        DDH_TMP_REQS_TXT=$FOL_LI/ddh/requirements.txt
        _pb "selected requirements file $DDH_TMP_REQS_TXT"
        "$VPIP" install --no-cache-dir -r "$DDH_TMP_REQS_TXT"
        _e $? "cannot install $DDH_TMP_REQS_TXT"
    else
        git clone $GH_REPO_DDH $F_CLONE_DDH --depth 1
        _e $? "cannot clone DDH"
    ...
    _pb "[ 98% ] installing script post rc-local"
    cp "$FOL_DDT"/_dt_files/script_from_rc_local.sh "$FOL_LI"
