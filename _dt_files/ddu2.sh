...
# -------------------------------------
# this does NOT get the latest version
# of DDH but the latest tagged one
# -------------------------------------


DDH_TAG=$(cd "$FOL_DDH" && git describe --tags --abbrev=0)
if [ -z "$DDH_TAG" ]
then
    # variable is empty
    echo "error: cannot find DDH latest tag, quitting"
    exit 1
fi


echo
_S="[ DDU ] detected DDH latest tag as $DDH_TAG"
_pb "$_S"
