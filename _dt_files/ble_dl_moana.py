...
from utils.ddh_shared import get_ddh_folder_path_dl_files, get_dl_folder_path_from_mac, create_folder_logger_by_mac
from utils.logs import lg_dds as lg
from mat.ble.ble_mat_utils import ble_mat_progress_dl, ael
...
class MoanaBle:
    ...
    async def connect(self, mac):
        ...
        try:
            self.cli = BleakClient(mac, adapter=self.h,
                                   adapterdisconnected_callback=cb_disc)
    ...
    async def file_info(self):
        ...
        if match is not None:
            ...
            try:
                self.offload_file_path = str(pathlib.Path(self.offload_file_folder) / self.offload_file_name)
    ...
    def generate_lowell_csv_files(self):
        ...
        # todo HP ---> test this file generation thing
        ...
        pt = v + p[:-4] + '_Temperature.csv'
        pp = v + p[:-4] + '_Pressure.csv'
    ...
    async def download_recipe(self, mac):
        ...
        if rv:
            print('moana could not connect to {}'.format(mac))
            return False
        status = True
...
async def ble_interact_moana(dl_folder, mac, h):
    lc = MoanaBle(dl_folder, h)
    try:
        ...
        print('[ BLE ] dl_moana_exception', ex)
        await lc.disconnect()
        return 1
...
# test
# ------
def main_moana(fol, _mac):
    # fol: dl_files_folder_path
    _h = 'hci0'
    _args = [fol, _mac, _h]
    ael.run_until_complete(ble_interact_moana(*_args))


if __name__ == '__main__':
    _mac = 'C9:3C:F8:37:E9:6A'
    d = get_ddh_folder_path_dl_files()
    d = str(d / _mac).replace(':', '-')
    main_moana(d, _mac)
